...
//
// Split from the HTTP client so the two rules that matter can be tested
// without a network: which epics a person is allowed to see, and which
// deliverables an epic carries.
...
export interface GitLabEpic {
  ...
  /** `Deliverable::*` labels only; everything else is dropped on the way in. */
...
/**
 * The `Deliverable::*` labels on an epic, in the canonical order.
 *
 * An epic carries labels Portal has no opinion about (workflow state,
  ...
  return {
    ...
    description:
      typeof raw.description === "string" && raw.description.trim()
        ? raw.description
        : null,
    deliverables: deliverablesFromLabels(labels),
